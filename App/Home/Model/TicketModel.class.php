...
class TicketModel extends Model
{
    ...
    //12406实时余票查询
    public function query4($from,$to,$date,$tt){
        $url = "http://apis.juhe.cn/train/yp";
        $params = array(
            "key" => $this->_app_key,//应用APPKEY(应用详细页查询)
            "dtype" => "",//返回数据的格式,xml或json，默认json
            "from" => $from,//出发站,如：上海虹桥
            "to" => $to,// 到达站,如：温州南
            "date" => $date,//出发日期，默认今日
            "tt" => $tt,//车次类型，默认全部，如：G(高铁)、D(动车)、T(特快)、Z(直达)、K(快速)、Q(其他)
        );
        $paramstring = http_build_query($params);
        $content = $this->_juhecurl($url,$paramstring);
        $result = json_decode($content,true);
    ...
    //12406订票：车次查询
    public function query5($from,$to,$date,$tt){
        $url = "http://apis.juhe.cn/train/ticket.cc.php";
        $params = array(
            "from" => $from,//出发站名称：如上海虹桥
            "to" => $to,//到达站名称：如温州南
            "date" => $date,//默认当天，格式：2014-07-11
            "tt" => $tt,//车次类型，默认全部，如：G(高铁)、D(动车)、T(特快)、Z(直达)、K(快速)、Q(其他)
            "key" => $this->_app_key,//应用APPKEY(应用详细页查询)
        );
        $paramstring = http_build_query($params);
    ...
    //火车票代售点查询
    public function query6($province,$city,$country){
        $url = "http://apis.juhe.cn/train/dsd";
        $params = array(
            "province" => $province,//省份,如：浙江
            "city" => $city,//城市，如：温州
            "county" => $country,//区/县，如：鹿城区
            "key" => $this->_app_key,//应用APPKEY(应用详细页查询)
            "dtype" => "",//返回数据的格式,xml或json，默认json
        );
        $paramstring = http_build_query($params);
