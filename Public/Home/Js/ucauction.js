...
function refreshList() {
	nTimeOut = 5;
	 ...
	 $.getJSON('/app?type=ajax&action=getperiodforuserhomeex', {'pids':inbidids} ,function(periods){
...
function refreshAuction(period) {
	...
	// 处理出价
	$('#myPriceShow_'+period.id).text(period.bidPrice);
	$('#myCountShow_'+period.id).text(period.Count);
